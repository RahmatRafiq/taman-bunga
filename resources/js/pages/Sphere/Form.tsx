...
export default function SphereFormPage() {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Spheres', href: route('sphere.index') },
        { title: isEdit ? 'Edit Sphere' : 'Create Sphere', href: '#' },
    ];

    const initialSphereFile =
        sphere?.media
            ?.filter((m) => m.collection_name === 'sphere_file')
            .map((m) => m.original_url) || [];
    const initialSphereImage =
        sphere?.media
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        let payload = { ...data };
        if (isEdit && !fileChangedRef.current) {
            delete payload.sphere_file;
        }
        if (isEdit && !imageChangedRef.current) {
            delete payload.sphere_image;
        }
        if (isEdit) {
            put(route('sphere.update', sphere!.id), { ...payload, preserveScroll: true });
        } else {
            post(route('sphere.store'), { ...payload, preserveScroll: true });
        }
    };
            ...
            <VirtualTourLayout>
                <div className="px-4 py-6">
                    ...
                    <form onSubmit={submit} className="space-y-6">
                            ...
                            <CustomSelect
                                ...
                                value={
                                    virtualTours
                                        ...
                                        .find((opt) => opt.value === data.virtual_tour_id) || null
                                }
                                onChange={(selected) => {
                                    setData(
                                        'virtual_tour_id',
                        ...
                        <div className="grid gap-2">
                            <Label>3D File (OBJ, GLTF, GLB, FBX)</Label>
                            <div
                                ref={fileRef}
                                className="dropzone border-dashed border-2 rounded p-4 dark:text-black"
                            />
                            <InputError message={errors.sphere_file} />
                        </div>
                        ...
                        <div className="grid gap-2">
                            ...
                            <div
                                ...
                                className="dropzone border-dashed border-2 rounded p-4 dark:text-black"
                            />
                            <InputError message={errors.sphere_image} />
                        </div>
