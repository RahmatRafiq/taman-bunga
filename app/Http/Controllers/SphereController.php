...
class SphereController extends Controller
{
    public function index(Request $request)
    {
        ...
        $userVirtualTours = VirtualTour::ownedBy($userId)->latest()->select('id', 'name')->get();
        $userVirtualTourIds = $userVirtualTours->pluck('id')->toArray();

        // Eager load virtualTour di spheres
        $spheres = match ($filter) {
            'trashed' => Sphere::onlyTrashed()->with('virtualTour')->whereIn('virtual_tour_id', $userVirtualTourIds)->get(),
            'all' => Sphere::withTrashed()->with('virtualTour')->whereIn('virtual_tour_id', $userVirtualTourIds)->get(),
            default => Sphere::with('virtualTour')->whereIn('virtual_tour_id', $userVirtualTourIds)->get(),
        };
    ...
    public function json(Request $request)
    {
        ...
        // Ambil virtual tour milik user login sekali saja
        $userVirtualTourIds = VirtualTour::ownedBy($userId)->pluck('id')->toArray();

        $query = match ($filter) {
            'trashed' => Sphere::onlyTrashed()->with('virtualTour')->whereIn('virtual_tour_id', $userVirtualTourIds),
            'all' => Sphere::withTrashed()->with('virtualTour')->whereIn('virtual_tour_id', $userVirtualTourIds),
            default => Sphere::with('virtualTour')->whereIn('virtual_tour_id', $userVirtualTourIds),
        };
    ...
    {
        $userId = request()->user()->id;
        $userVirtualTours = VirtualTour::ownedBy($userId)->latest()->get();
